...
SELECT AVG(lifeexpectancy) AS avg_life_expect_in_S_America
FROM country
WHERE continent = 'South America'
-- 5. The sum of the population of all cities in California.
-- (total population of all cities in California: 16716706)
SELECT SUM(population) AS sum_pop_in_cali
FROM city
...
SELECT SUM(surfacearea) AS surfacearea_total, continent
FROM country
GROUP BY continent
ORDER BY surfacearea_total DESC
-- 14. The highest population density (population divided by surface area) of all 
-- countries in the world. 
-- (highest population density in world: 26277.7777)
SELECT MAX(population/surfacearea) AS density
FROM country
-- 15. The population density and life expectancy of the top ten countries with the 
-- highest life expectancies in descending order. 
-- (highest life expectancies in world: 83.5, 166.6666, "Andorra")
SELECT lifeexpectancy, MAX(population/surfacearea) AS density, name
FROM country 
...
SELECT MIN(lifeexpectancy), AVG (lifeexpectancy), MAX(lifeexpectancy), continent
FROM country
WHERE lifeexpectancy IS NOT NULL
GROUP BY continent
